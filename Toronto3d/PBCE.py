...
from Toronto3d.ProjectionConvolution import get_pretrained_res2net50_unet, Res2NetUNet
...
class PCENet(nn.Module):
    def __init__(
        ...
        # Instantiate the Res2NetUNet backbone using the factory initialization function
        if use_pretrained:
            self.encoder_2d = get_pretrained_res2net50_unet(in_channels=in_point_feat, out_channels=128)
        else:
            self.encoder_2d = Res2NetUNet(in_channels=in_point_feat, out_channels=128, baseWidth=26, scale=4)
    ...
    def forward(self, p_local, p_context, bi_local, bi_context, batch_size, window_centers, labels=None):
        ...
        # Padded target automatically tracks multiples of 32 (e.g. 96x96 canvas layout)
        ...
        q_2d_xy = Res2NetUNet.query_context_features(E_2d_xy, pix_local_xy, bi_local)
        q_2d_yz = Res2NetUNet.query_context_features(E_2d_yz, pix_local_yz, bi_local)
        q_2d_xz = Res2NetUNet.query_context_features(E_2d_xz, pix_local_xz, bi_local)
