...
from utils import calculate_class_weights, train_one_epoch, validate, pce_collate
...
def main():
    ...
    if not all_csv_files: raise RuntimeError(f"No CSVs in {TRAIN_CSV_DIR}")

    kf = KFold(n_splits=NUM_FOLDS, shuffle=True, random_state=42)

    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(all_csv_files)):
        if(fold_idx != 2):
            continue
        print(f"\nFold {fold_idx + 1}/{NUM_FOLDS} | Train: {len(train_idx)} files | Val: {len(val_idx)} files")
